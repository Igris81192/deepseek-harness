...
**打包启动。** 打包后的 `.app` 从自身 `node_modules` 启动 web profile：`main.ts` 在 `app.isPackaged` 时传入 `bareModuleBaseUrl`，且应用包以未打包目录形式发布（`electron-builder.yml` 中 `asar: false`）——Electron 的 Node 无法穿过 `~/.dsh/profiles/node_modules` 指向 asar 内部的符号链接（asar 目标的 realpath 以 ENOTDIR 失败），因此 node_modules 必须是真实目录，同样的符号链接查找才能生效。应用的 `dependencies` 直接声明完整宿主面——应用闭包无法覆盖的服务定义与 peer 包（如 `@deepseek-ai/cordis-plugin-group` 与 `@deepseek-ai/dsh-invariants`）——因为 electron-builder 只打包声明的依赖图。Finder 启动不传脚本参数时，`main.ts` 还会提供 `process.argv[1]`：profile-boot 为长驻表面挂载的 watch-only HMR 实例会解析它来跟踪重载依赖。
