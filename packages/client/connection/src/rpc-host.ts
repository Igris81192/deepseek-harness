...
export class HostConnectionService extends Service implements HostConnectionHandle {
   ...
   * fallback (privileged-method loopback pin, events upgrade fence, API Proxy
   * unary/respond). This is the exact handler the HTTP bridge serves, exposed
   * as a plain Fetch handler so transport-independent adapters (an Electron
   * IPC bridge) reuse the same fence without a webserver. Composing is cheap
   * and the interceptor table is read per request, so adapters may hold one
   * handler for the host lifetime.
   * @returns a Fetch handler selecting exactly one target per request.
   */
